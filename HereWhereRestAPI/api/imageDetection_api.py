from flask import Flask, request
from flask_restful import Api, Resource
from PIL import Image
import base64
import io
from ultralytics import YOLO
import os
import uuid, requests
from google.cloud import translate_v2 as translate

class ImageDetection(Resource):
    def __init__(self):
        # YOLO 모델 로드
        self.model = YOLO('../model/best.pt')
        # Google Cloud Translation 클라이언트 생성
        self.translate_client = translate.Client()

    def post(self):
        data = request.get_json()
        base64_str = data.get('image')
        base64_str = base64_str.split(',')[-1]  # 데이터 URI 스키마 제거
        # Base64 문자열을 이미지로 디코딩
        img_bytes = base64.b64decode(base64_str)
        img_io = io.BytesIO(img_bytes)
        img = Image.open(img_io)

        # 이미지 이름 생성 (임시 파일로 저장)
        img_name = f'temp_{uuid.uuid4()}.jpg'
        img = img.convert("RGB")  # 이미지를 RGB 모드로 변환
        with open(img_name, 'wb') as f:
            img.save(f)

        # 모델에 이미지를 입력하여 예측 수행
        results = self.model.predict([img_name], save=True, line_width=1, conf=0.5)
        result = results[0]  # 첫 번째 결과만 사용

        # 결과에서 클래스 인덱스와 확신도 가져오기
        class_indices = result.boxes.cls
        confidences = result.boxes.conf
        class_confidences = list(zip(class_indices.tolist(), confidences.tolist()))

        # 확신도가 0.9 이상인 레이블만 선택
        selected_labels = [result.names[int(idx)] for idx, conf in class_confidences if conf >= 0.9]
        selected_labels = list(set(selected_labels))  # 중복 제거

        search_image_api_url = os.getenv('PYTHON_API_URL')+'/searchImage'

        with open(img_name, 'rb') as f:
            files = {'image': f}
            response = requests.post(search_image_api_url, files=files)

        # 임시 파일 삭제
        os.remove(img_name)

        if response.status_code == 200:
            descriptions = response.json().get('descriptions', [])
            # descriptions 리스트의 각 항목을 한국어로 번역
            translated_descriptions = []
            for description in descriptions:
                result = self.translate_client.translate(description, target_language='ko')
                translated_descriptions.append(result['translatedText'])
            #5개만 출력
            translated_descriptions = translated_descriptions[:5]
            selected_labels.extend(translated_descriptions)

        return {'labels': selected_labels}
